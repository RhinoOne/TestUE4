...
void ATestProjectCharacter::BeginPlay()
{
	...
	if(URMManagerID* ManagerId = URMManagerID::GetManagerIdInstance())
	{
		ManagerId->GetAllActorsID(CurrentWorld);
		ManagerId->CheckingUniqueIdentifier();
	}
}
...
void ATestProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind fire event
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &ATestProjectCharacter::OnFire);
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<ATestProjectProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
...
void ATestProjectCharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void ATestProjectCharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void ATestProjectCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
bool ATestProjectCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &ATestProjectCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &ATestProjectCharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &ATestProjectCharacter::TouchUpdate);
		return true;
	}
